...
public class Hangman extends ConsoleProgram {
	...
	private void setUpConsole() {
		...
		String fileLine = readFile(); // chooses a random line from txt file.
		int spliter = fileLine.indexOf('_');
		word = fileLine.substring(0, spliter);
		definition = fileLine.substring(spliter + 1);
		FinalWord = word; // saves full version of word,because we change "word" variable during the game.
		// println(word);
		/*
		 * sets up the string which appears during the game. at first,it consists of
		 * only '-' s.
		 */
		...
		canvas.displayDefinition(definition);
	}
	...
	private char readChar(String s) {
		...
		/*
		 * if the character is a small letter, we make it a big one and return it as an
		 * inputed letter.
		 */
		if (c >= 'a' && c <= 'z') {
			c -= 'a' - 'A';
